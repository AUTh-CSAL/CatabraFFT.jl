...
function benchmark_fft_over_range(xs::Vector; ctype=ComplexF64, plan_type=FFTW.MEASURE, save=false, msg="")
    gflops_catabra = []
    gflops_fftw = []
    fftw_time = []
    mixed_radix_time = []
...
for b in [2 3 5 7 10]
    ...
    for ctype in [ComplexF32, ComplexF64]
        benchmark_fft_over_range(xs; ctype, plan_type=fftwplan, save, msg="FFT-$b")
    end
end
println("Done!")